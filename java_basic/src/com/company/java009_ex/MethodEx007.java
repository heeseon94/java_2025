...
public class MethodEx007 {
	public static void who_are_you(String [][] users){  //아이디를입력받아서 나라조회 
		...
		//처리+출력
		//if (내가 입력받은 id가 "aaa"같니?) / {aaa는 한국사람입니다.}		
		//if (내가 입력받은 id가 "bbb"같니?) / {bbb는 호주사람입니다.}
		//if (내가 입력받은 id가 "ccc"같니?) / {ccc는 중국사람입니다.}
...
//		who_are_you(users); 
	...
	public static void who_pass_change(String [][] users) {
		...
		//입력
		System.out.print("아이디를 입력해주세요 > "); temp_id = scanner.next();
		System.out.print("비밀번호를 입력해주세요 > "); temp_pass = scanner.next();
		
		//처리
		//1) 사용자인증 1 - if(입력받은 아이디 "aaa"/ 입력받은 비번과 111같다면) {해당유저의 0층}
		//              - if(입력받은 아이디 "bbb"/ 입력받은 비번과 222같다면) {해당유저의 1층}      
		//1) 사용자인증 2 - if(temp_id.equals(users[0][0]) && temp_pass.equals(users[0][1])) { 0층 }
		//              - if(temp_id.equals(users[1][0]) && temp_pass.equals(users[1][1])) { 0층 }
		//2) 맞다면 새로운 비번입력 - 변경하실 비밀번호를 입력해주세요 123
		int find=-1;
		for(int ch=0; ch<users.length; ch++) {
