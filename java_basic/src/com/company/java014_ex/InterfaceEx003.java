...
/*
 -----------------------------------------------------
 [method : 정보] Vehicle.class, MoterCycle, Car, Driver, public InterfaceEx003 //#1 설계도
 -----------------------------------------------------
 [heap : 동적]							|[stacks : 지역]
 										  driver.drive(mo ) ▶ 1번지.dirve(Vehicle v     ){v.run();}
 															▶1)Vehicle v=mo;    ▶2)부모=자식 @Override 자식메서드  mo.run() 										  
 										  driver.drive(car) ▶ 1번지.dirve(Vehicle v     ){v.run();}
 															▶1)Vehicle v=car;   ▶2)부모=자식 @Override 자식메서드 car.run()
3번지		MotorCtcle {run}			← mo		3번지
2번지 		Car        {run}			← car		2번지
