...
public class Score2Process {
	...
	public void process_avg(Score2[] std) { // 주소따라가서 데이터 수정하겠습니다.
		...
		//std[0].setavg = (std[0].setkor + std[0].seteng + std[0].setmath +)/3.0
	...
	public void process_pass(Score2[] std) {
		...
		//ver2. not nisible → private - getters/setters
		//std[0].setPass (std[0].getAvg() >=60? "합격" : "불합격");
		for(int i=0; i<std.length; i++) {std[0].setPass (std[0].getAvg() >=60? "합격" : "불합격");}
	}
